...
const FACT_RULES = [
  [/^(first|given|preferred) ?name$/i, 'firstName'], [/^(last|family|sur) ?name$/i, 'lastName'], [/^(full |your )?name$/i, 'name'],
...
// Walk the wizard. Returns { pages: [{ title, url, questions, identity, files, filled, skipped }], questions, files, identity, stoppedAt }.
export async function walkWizard(page, { facts = {}, cvPath = null, log, maxPages = 8 } = {}) {
  ...
  for (let n = 0; n < maxPages; n++) {
    ...
    for (const f of collected.fields) {
      if (!f.required || f.value || f.idx == null) continue;
      const loc = page.locator(`[data-co-idx="${f.idx}"]`);
      try {
        if (f.type === 'file') { const wantsCv = /resume|cv|curriculum/i.test(f.label + ' ' + f.name); if (wantsCv && cvPath && existsSync(cvPath)) { await loc.setInputFiles(cvPath); filled.push(`${f.label} ← tailored CV`); } else skipped.push(f.label); continue; }
        const fact = factFor(f.label, facts);
        if (f.type === 'select') { const opt = pickOption(f.options ?? [], fact); if (opt) { await loc.selectOption({ label: opt }); filled.push(`${f.label} ← ${opt}`); } else skipped.push(f.label); continue; }
        if (['text', 'textarea', 'email', 'tel', 'url', 'number'].includes(f.type)) { if (fact) { await loc.fill(fact); filled.push(`${f.label} ← ${fact.slice(0, 40)}`); } else skipped.push(f.label); continue; }
  ...
  return { pages, questions: [...seenQ.values()], stoppedAt };
}
