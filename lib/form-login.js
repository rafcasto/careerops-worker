...
export function answerFor(label, answers = []) {
  ...
  const exact = answers.find((a) => normalizeQuestion(a.question) === l); if (exact?.answer) return exact.answer;
  const loose = answers.find((a) => { const q = normalizeQuestion(a.question); return q.length > 12 && (l.includes(q) || q.includes(l)); });
...
export async function walkWizard(page, { facts = {}, cvPath = null, answers = [], mode = 'reach', log, maxPages = 8 } = {}) {
  ...
  for (let n = 0; n < maxPages; n++) {
    ...
    for (const f of collected.fields) {
      const wanted = mode === 'fill' ? (!f.value && !(f.checked?.length)) : (f.required && !f.value);
      if (!wanted) continue;
