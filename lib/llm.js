...
import { isClaudeCliTag, cliModelFromTag, teachViaCli, claudeCliAvailable } from './claude-cli.js';

// A "claude-cli[:model]" tag on any agent sends the same system + user turn to Claude Code
// (headless, no tools) instead of n8n/Ollama. Same return shape, so jobs don't care.
async function runClaudeCli({ cfg, messages, log, progress, cancelled, format }) {
  if (!(await claudeCliAvailable())) throw new Error('This agent is set to Claude Code, but the CLI is not logged in on the Pi — run `claude auth login` as the worker user, or pick an Ollama model in Admin → CareerOps → Models');
  ...
  await progress(`waiting for Claude Code${model ? ` (${model})` : ''}…`);
  const r = await teachViaCli({ system, user, model, log, cancelled });
  let content = r.text;
  if (format === 'json') { const a = content.indexOf('{'), b = content.lastIndexOf('}'); if (a >= 0 && b > a) content = content.slice(a, b + 1); }
  return { content, via: 'claude-cli', usage: { prompt: r.usage.input, completion: r.usage.output }, doneReason: r.stopReason === 'end_turn' ? 'stop' : r.stopReason, durationMs: r.durationMs };
}
...
export async function runAgent({ env, agent, cfg, messages, log, progress, cancelled, optionOverrides = {}, format = null }) {
  if (isClaudeCliTag(cfg.model)) return runClaudeCli({ cfg, messages, log, progress, cancelled, format });
  ...
  try {
    const r = await post(hook, { agent, model: cfg.model, options, messages, ...(format ? { format } : {}) }, { cancelled });
    if (r.status === 200) {
  ...
  let lastProgress = 0;
  await post(`${env.ollama}/api/chat`, { model: cfg.model, messages, options, stream: true, ...(format ? { format } : {}) }, {
    cancelled,
    onLine: (line) => {
      try {
