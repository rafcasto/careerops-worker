...
export async function updateReport(uid, jobId, patch) {
  await firestore().doc(`users/${uid}/careerOpsReports/${jobId}`).set(patch, { merge: true });
}
...
// users/{uid}/careerOpsAnswers — the member's answer bank (owner-written from the Apply screen).
export async function listAnswers(uid) {
  const snap = await firestore().collection(`users/${uid}/careerOpsAnswers`).get();
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
export async function touchAnswers(uid, ids) {
  if (!ids.length) return;
  const b = firestore().batch(); const now = Date.now();
  for (const id of ids) b.set(firestore().doc(`users/${uid}/careerOpsAnswers/${id}`), { usedAt: FieldValue.arrayUnion(now), lastUsedAt: now }, { merge: true });
  await b.commit();
}
...
// users/{uid}/careerOps/portalsStatus — per-company outcome of the last scan (Setup shows it).
export async function savePortalsStatus(uid, doc) {
  await firestore().doc(`users/${uid}/careerOps/portalsStatus`).set({ ...doc, at: Date.now() });
}
