...
export function collectFieldsInPage() {
  ...
  for (const el of els) {
    ...
    if (type === 'radio' || type === 'checkbox') {
      ...
      if (el.getAttribute('role') === 'switch') { const lbl = (el.id && document.querySelector(`label[for="${CSS.escape(el.id)}"]`)?.textContent) || labelFor(el); out.push({ idx: k++, type: 'switch', name: el.name || el.id || '', label: clean(lbl), required, value: el.checked ? 'on' : '' }); continue; }
    ...
    const combobox = el.getAttribute('role') === 'combobox' || el.getAttribute('aria-autocomplete') === 'list' || /react-select__input|autosuggest__input|autocomplete/i.test(String(el.className || '')) || !!el.closest('[role="combobox"]');
    const f = { idx: k++, type, name: el.name || el.id || '', label: labelFor(el), required, maxLength: el.maxLength > 0 ? el.maxLength : null, value: type === 'file' ? '' : String(el.value ?? '').slice(0, 200), ...(combobox ? { combobox: true, selected: clean((el.closest('[class*="value-container"]') ?? el.closest('[class*="control"]') ?? el.closest('[class*="select"]'))?.querySelector('[class*="single-value"], [class*="multi-value"], [class*="selected"]')?.textContent) || '' } : {}) };
...
const IDENTITY = /^(legal |preferred |your )?(first|last|full|given|family|middle|sur)?\s*name:?$|^title:?$|\be-?mail\b|\bphone\b|\bmobile\b|linkedin|github|portfolio|website|^(city|town|suburb|region|state|region \/ state|county|province)$|\bcountry\b|\baddress\b|post(al)? ?code|\bzip\b|date of birth|^dob$/i;
...
export function classifyFields(collected, url) {
  ...
  for (const f of fields) {
    ...
    if (f.type === 'switch' && !f.required) continue;   // consent toggles that aren't required are the member's call
  ...
  const atsHint = /myworkdayjobs/.test(host) ? 'Workday' : /successfactors|jobs2web/.test(host) ? 'SuccessFactors' : /csod\.com/.test(host) ? 'Cornerstone' : /greenhouse/.test(host) ? 'Greenhouse' : /lever\.co/.test(host) ? 'Lever' : /ashbyhq/.test(host) ? 'Ashby' : /smartrecruiters/.test(host) ? 'SmartRecruiters' : /eightfold/.test(host) ? 'Eightfold' : /snaphire|\/candidate\/application\//i.test((collected.url || url) + ' ' + (collected.text ?? '').slice(0, 2000)) ? 'SnapHire' : null;
  ...
  return { fields, questions, identity, files, needsAccount, atsHint, finalUrl: collected.url || url, title: collected.title ?? '', note, draftUrl: draftUrlFor(collected.url || url, atsHint) };
}

// Where the portal lists the member's in-progress application (the wizard link with its tracker id works too).
export function draftUrlFor(url, atsHint) {
  try { const u = new URL(url); if (/snaphire|\/candidate\//i.test(u.href) || /asbgroup/.test(u.hostname)) return `${u.origin}/candidate`; if (atsHint === 'Workday') return `${u.origin}${u.pathname.split('/').slice(0, 3).join('/')}/userHome`; return url; } catch { return url; }
}
