...
export function collectFieldsInPage() {
  ...
  for (const el of els) {
    ...
    if (type === 'radio' || type === 'checkbox') {
      ...
      const g = groups.get(name) ?? { type: type === 'radio' ? 'radio' : 'checkbox', name, label: '', options: [], required: false };
      ...
      g.options.push(clean(optLabel)); g.required ||= required;
      groups.set(name, g); continue;
    }
    el.dataset.coIdx = String(out.length);   // handle for the signed-in walker (lib/form-login.js)
    const f = { idx: out.length, type, name: el.name || el.id || '', label: labelFor(el), required, maxLength: el.maxLength > 0 ? el.maxLength : null, value: type === 'file' ? '' : String(el.value ?? '').slice(0, 200) };
