...
export function collectFieldsInPage() {
  ...
  for (const el of els) {
    ...
    const f = { idx: k++, type, name: el.name || el.id || '', label: labelFor(el), required, maxLength: el.maxLength > 0 ? el.maxLength : null, value: type === 'file' ? '' : String(el.value ?? '').slice(0, 200), ...(combobox ? { combobox: true, selected: clean(el.closest('[class*="select"]')?.querySelector('[class*="single-value"], [class*="selected"]')?.textContent) || '' } : {}) };
