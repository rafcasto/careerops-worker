...
export async function researchViaCli({ system, user, model = '', maxSearches = 6, maxTurns = 14, timeoutMs = 15 * 60 * 1000, log, cancelled }) {
  ...
  const m = model || CLI_MODEL;
  const args = ['-p', '--output-format', 'json', '--allowedTools', 'WebSearch,WebFetch', '--max-turns', String(maxTurns), '--append-system-prompt', sys, ...(m ? ['--model', m] : [])];
...
// Model tags the admin can pick in Compass → Models. "claude-cli" = the CLI's default model;
// "claude-cli:<alias>" pins one (the CLI accepts sonnet / opus / haiku or a full model id).
export const CLAUDE_CLI_TAG = /^claude-cli(?::([A-Za-z0-9._-]+))?$/;
export const isClaudeCliTag = (tag) => CLAUDE_CLI_TAG.test(String(tag ?? ''));
export const cliModelFromTag = (tag) => String(tag ?? '').match(CLAUDE_CLI_TAG)?.[1] || CLI_MODEL || '';
export const CLAUDE_CLI_MODELS = [
  { name: 'claude-cli', label: 'Claude Code · default model', family: 'claude' },
  { name: 'claude-cli:sonnet', label: 'Claude Code · Sonnet (fast)', family: 'claude' },
  { name: 'claude-cli:opus', label: 'Claude Code · Opus (best)', family: 'claude' },
  { name: 'claude-cli:haiku', label: 'Claude Code · Haiku (cheapest)', family: 'claude' },
];

export async function teachViaCli({ system, user, model = '', timeoutMs = 20 * 60 * 1000, log, cancelled }) {
  const bin = await claudeCliBin();
  ...
  const m = model || CLI_MODEL;
  const args = ['-p', '--output-format', 'json', '--allowedTools', '', '--max-turns', '1', '--append-system-prompt', sys, ...(m ? ['--model', m] : [])];
  const { stdout, stderr, code } = await new Promise((resolve) => {
    const child = execFile(bin, args, { cwd: SCRATCH, timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024, env: { ...process.env, CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC: '1' } },
      (err, stdout, stderr) => resolve({ stdout: stdout ?? '', stderr: stderr ?? '', code: err ? (err.code ?? 1) : 0 }));
    child.stdin.end(user);
