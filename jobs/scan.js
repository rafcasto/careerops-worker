...
// Before pass 1, every company no provider claims goes through lib/ats-resolve.js: the careers
// page is fetched and fingerprinted for the job board behind it (Workday link, branded
// SuccessFactors host, …). A confirmed board is scanned right away AND offered on the Setup row
// as a one-click suggestion — the member's own config is never rewritten.
...
import { resolveCompany, loadCareerOpsTools } from '../lib/ats-resolve.js';
...
// portals.yml with the resolved boards swapped in for their companies (name match, case-insensitive).
export function applyResolvedToPortalsYaml(portalsYaml, resolved) {
  if (!resolved.length) return portalsYaml;
  const doc = yamlLoad(portalsYaml) ?? {};
  const list = Array.isArray(doc.tracked_companies) ? doc.tracked_companies : [];
  for (const r of resolved) {
    const e = list.find((x) => x && String(x.name ?? '').trim().toLowerCase() === r.name.toLowerCase());
    if (!e) continue;
    e.careers_url = r.suggested.careersUrl;
    if (r.suggested.provider) e.provider = r.suggested.provider; else delete e.provider;
    if (r.suggested.apiUrl) e.api = r.suggested.apiUrl; else delete e.api;
  }
  return '# Generated by JobHacker Compass — boards resolved by the Scout are swapped in for this scan only.\n' + yamlDump(doc, { lineWidth: 200 });
}
...
export async function run({ job, env, log, progress, cancelled }) {
  ...
  // ---- pass 0: find the board behind each unclaimed careers page ----
  const unresolved = classified.filter((c) => c.method === 'page' && !c.error);
  const resolved = [];
  if (unresolved.length) {
    let tools = null;
    try { tools = await loadCareerOpsTools(env.repo); } catch (e) { await log(`board resolver unavailable (${e.message}) — skipping`); }
    for (const [i, c] of tools ? unresolved.entries() : []) {
      if (await cancelled()) return null;
      await progress(`looking for ${c.name}'s job board (${i + 1}/${unresolved.length})`);
      const t1 = Date.now();
      let s = null;
      try { s = await resolveCompany(c, { resolve: tools.resolve, probe: tools.probe, log }); } catch (e) { await log(`${c.name}: board lookup failed — ${e.message}`); }
      const st = status.get(c.careersUrl);
      if (s?.provider) {
        Object.assign(c, { method: 'board', provider: s.provider, resolvedFrom: s.via });
        Object.assign(st, { method: 'board', provider: s.provider, suggested: { careersUrl: s.careersUrl, provider: s.provider, apiUrl: s.apiUrl, live: s.live, via: s.via } });
        resolved.push({ name: c.name, suggested: st.suggested });
        await log(`${c.name}: found its job board — ${s.provider} at ${s.careersUrl}${s.live != null ? ` (${s.live} posting${s.live === 1 ? '' : 's'})` : ''} in ${Math.round((Date.now() - t1) / 1000)}s · accept it in Setup to keep`);
      } else if (s?.hint) { st.hint = s.hint; await log(`${c.name}: ${s.hint}`); }
      else await log(`${c.name}: no job board found behind ${c.careersUrl} — reading the page in the browser`);
    }
  }
  await writeFile(join(root, 'portals.yml'), applyResolvedToPortalsYaml(portals, resolved) + '\n', 'utf8');
  const boards = classified.filter((c) => c.method === 'board');
  await log(`${companies.length} compan${companies.length === 1 ? 'y' : 'ies'}: ${boards.length} on a job board (${boards.map((c) => `${c.name}:${c.provider}${c.resolvedFrom ? '*' : ''}`).join(', ') || '—'}), ${classified.length - boards.length} via browser${resolved.length ? ' · * = found by the Scout' : ''}`);
